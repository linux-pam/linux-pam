...
shift_buf(char *mem, int from)
{
	...
	++mem;
    _pam_override_n(mem, PAM_GROUP_BUFLEN - (mem - start));
    return mem;
}
	   ...
	   const char *conf_filename)
{
	...
	*from = 0;
	...
	if (i < 0) {
	    ...
	    close(fd);
	    _pam_override_n(*buf, PAM_GROUP_BUFLEN);
	    _pam_drop(*buf);
	    *state = STATE_EOF;
	    return -1;
	} else if (!i) {
	...
	*state = STATE_EOF;
    ...
    _pam_override_n(to, PAM_GROUP_BUFLEN - (to - *buf));
			 ...
			 const char *tty, const char *user)
{
    ...
    if (grps) {                                          /* tidy up */
	_pam_override_n(grps, sizeof(gid_t) * blk_size(no_grps));
	_pam_drop(grps);
	no_grps = 0;
    }
